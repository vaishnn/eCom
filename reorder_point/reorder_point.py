class reorder:
    def __init__(self):
        ...
        self.lead_time = None
        self.carrying_cost = None
        self.stockout_cost = None
        self.cash_conversion_cycle = None
        self.days_inventory_holding = None
        self.days_sales_outstanding = None
        self.days_playable_outstanding = None
        self.economic_order_quantity = None
...
def thoughput_data_generation(
    ...
    TOTAL_MONTHLY_MIN = 80,
    TOTAL_MONTHLY_MAX = 120,
    N_YEARS = 2,
    ...
    daily_sales = pd.DataFrame(index=dates, columns=["orders"], data=0) #type: ignore
    ...
    for month_start in pd.date_range(start=START_DATE,
        ...
        for i in range(N_CUSTOMERS):
            ...
            order_day = base_days[i] + day_offset
            ...
            if order_date in daily_sales.index:
                daily_sales.loc[order_date] += order_quantity
